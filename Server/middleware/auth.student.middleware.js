    ...
    if (token) {
        ...
        jwt.verify(token, process.env.TOKEN_SECRET, async (err, decodedToken) => {
            ...
            // si le token match:
                ...
                res.locals.student = student; // on passe ce user ds le res.locals.user: var qui n'est dispo que ds une vue/page en particulier
                next(); // On continue le traitement de la requête
            }
        })
...
module.exports.requireAuthStudent = (req, res, next) => {
    //On stocke le(éventuel) cookie de l'utilisateur dans une const
    const token = req.cookies.jwt;
    //S'il y a un token
    if (token){
